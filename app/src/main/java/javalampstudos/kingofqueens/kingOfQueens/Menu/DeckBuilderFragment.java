...
//    private int[] page8 = {54,55,56,57,58,59,60}; //support2
    ...
    private int[] card7 = {7,15,23,31,39,-1,52,60};
...

    {
        ...
        switch(page)
        {
            ...
            case 8:
                ...
                canvas.drawBitmap(library.supportCards.get(14).getBitmap(),null,card7Rect,null);
                canvas.drawBitmap(cardbackBitmap,null,card8Rect,null);
                break;
        }
        ...
        for (int i = 0; i < input.MAX_TOUCH_POINTS; i++) {
            if (input.isTouchDown(i)) {
                ...
                if (page!=1) //Arrow does not appear on first page
                {
                ...
                if ((page!=8)||((page==6)&&(currentNumberOfPages!=6))) //Arrow does not appear on last page
                {
